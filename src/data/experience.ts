...
export const experiences: Experience[] = [
  {
    ...
    narrative:
      "At LexisNexis I joined a working system at a moment of scaling. I led the move to async processing patterns that eliminated some of the biggest performance bottlenecks, helped redesign how my team's services communicate, and rebuilt the data-layer paths I owned for speed. The platform now runs at nearly double the speed, holds 99.95% uptime, and deployments happen without anyone holding their breath.\n\nSix teams share one codebase, 40+ engineers across them. On my team I work full stack end to end — writing and optimizing complex SQL queries that cut database response times by 70%, designing caching strategies that doubled application speed, building the API layer that connects it all. When something on my watch is slow, unreliable, or blocking users, I find it, fix it, and make sure it stays fixed.\n\nThe exception-handling middleware I implemented catches errors and applies PII masking bidirectionally — masking for external API consumers, unmasking for trusted internal apps. Caching is distributed Redis so the cache survives service restarts and serves all instances from one source. Asynchronous workloads run through RabbitMQ, handling six-figure daily event volume. Resilient retry mechanisms via Polly framework hold 99.95% API availability and reduced errors by 82%.\n\nOn the data side, the most distinctive piece of work was an ETL feature where users upload entities through Excel. I initiated the database design, finalized the schema with the architect, and worked with the DBA on validation — shipping a main table and a validation stored procedure carrying the business logic.\n\nOn the frontend, I led the major-version Angular migration alongside the GraphQL framework upgrade — refactoring 30+ Material components in the process. Client-side GraphQL with Apollo dropped query execution from 200 ms to 120 ms. Two framework moves running in parallel.\n\nOn my own team — closer to ten — code reviews aren’t bug-hunts. They’re how you teach the next senior engineer what done looks like: how to write code that doesn’t come back as a bug ticket, how to design for the engineer who reads it six months later, how to make technical decisions that hold up under production reality. Post-release defects dropped 18%. Domain-driven design workshops shifted how the team thinks — business needs, technical architecture, and the boundaries between them get named before they become load-bearing. New capabilities ship 40% faster because architecture and product speak the same language now.",
    decisions: [
      {
        afterParagraph: 2,
  ...
  },
  {
    ...
    narrative:
      "Where it started. Built full-stack applications using the .NET web stack serving 3K+ daily users — owned everything from database design to the responsive front-end, shipping features that worked across browsers and devices.\n\nWrote complex SQL integrating 25+ stored procedures and optimized query performance, reducing execution time by 40%. This was where I learned that the database is never just a storage layer — it’s an engineering decision. Mastered advanced C# concepts — LINQ, custom generic constraints, extension methods. Created reusable UI components and established a component-driven development approach that accelerated feature delivery by 40% — introduced a pattern the team continued using long after I moved on.\n\nDesigned middleware solutions that enabled seamless error tracking integration, improving system observability by 70%. Built the foundation for how the team understood and responded to production issues. The thinking learned here — how to model data, how to debug across layers, how to ship something a team could actually maintain — is still the foundation I build on.",
    decisions: [
      {
        afterParagraph: 1,
